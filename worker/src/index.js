...
async function recognizeWithOpenAI({ imageDataUrl, cards }, env) {
  ...
  const prompt = [
    ...
    "If printed Japanese questions are visible on the sheet, use them as part of the matching signal together with the question number.",
    "Read only the student's handwritten answer in the answer box on the right side of each question row.",
    "Do not copy the printed practice vocabulary section as the student's answer, even if it contains the expected English word.",
